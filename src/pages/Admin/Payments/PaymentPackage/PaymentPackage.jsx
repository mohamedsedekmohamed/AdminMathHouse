...
const PaymentPackage = () => {
  ...
  const { data, loading: getLoading, error, refetch } = useGet(
    `/api/admin/payment/package-buy-requests?${queryParams}`
  );
  ...
  const handleConfirmAction = async () => {
    try {
      const url = `/api/admin/payment/package-buy-requests/${modal.requestId}/reply`;
...

    {
      ...
      render: (_, row) => (
        <div className="flex flex-col">
          ...
          <span className="text-xs text-muted-foreground">
            {row.package?.type} • {row.package?.number} items
          </span>
        </div>
      ),
    },
          ...
          <div className="bg-card w-full max-w-md rounded-2xl shadow-xl border border-border overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-muted-foreground text-sm mb-4">
                {modal.actionType === "approve"
                  ? "Are you sure you want to approve this package purchase?"
